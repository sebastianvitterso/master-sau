...
class FusionModel(nn.Module):
    ...
    def forward(self, x_rgb, x_ir, augment=False, profile=False, visualize=False):
        if augment:
            return self._forward_augment(x_rgb) #, x_ir  # augmented inference, None
        return self._forward_once(x_rgb, x_ir, profile, visualize)  # single-scale inference, train
    ...
    def _forward_once(self, x_rgb, x_ir, profile=False, visualize=False):
        ...
        for m in self.backbone_ir:
            if m.f != -1:  # if not from previous layer
                x_ir = y_ir[m.f] if isinstance(m.f, int) else [x_ir if j == -1 else y_ir[j] for j in m.f]  # from earlier layers
            x_ir = m(x_ir)
