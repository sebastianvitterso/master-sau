...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        ir_files = rgb2ir_paths(images)


        ni, nir = len(images), len(ir_files)

        self.img_size = img_size
        self.stride = stride
        self.files = images
        self.ir_files = ir_files
        self.nf = ni # + nir  # number of files
    ...
    def __next__(self):
        ...
        ir0 = cv2.imread(ir_path)  # BGR
        assert ir0 is not None, 'Image Not Found ' + ir_path

        # Padded resize
        img = letterbox(img0, self.img_size, stride=self.stride, auto=self.auto)[0]
        ir = letterbox(ir0, self.img_size, stride=self.stride, auto=self.auto)[0]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, ir, labels = load_mosaic(self, index)
            shapes = None

            # MixUp augmentation
            if random.random() < hyp['mixup']:
                img, ir, labels = mixup(img, ir, labels, *load_mosaic(self, random.randint(0, self.n - 1)))
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                im = F.interpolate(img[i].unsqueeze(0).float(), scale_factor=2., mode='bilinear', align_corners=False)[
                    0].type(img[i].type())
                im_ir = F.interpolate(ir[i].unsqueeze(0).float(), scale_factor=2., mode='bilinear', align_corners=False)[
                    0].type(ir[i].type())
                l = label[i]
            else:
                im = torch.cat((torch.cat((img[i], img[i + 1]), 1), torch.cat((img[i + 2], img[i + 3]), 1)), 2)
                im_ir = torch.cat((torch.cat((ir[i], ir[i + 1]), 1), torch.cat((ir[i + 2], ir[i + 3]), 1)), 2)
...
def load_mosaic(self, index):
    ...
    labels4, segments4, ir_segments4 = [], [], []
    s = self.img_size
    ...
    for i, index in enumerate(indices):
        ...
        ir, _, (ir_h, ir_w) = load_ir(self, index)
    ...
    img4, ir4, labels4 = random_perspective(img4, ir4, labels4, segments4,
